...
export interface ObserverInstanceCallbackOptions {
  expose: ExposeTime;
  moduleName: string;
}

export type ObserverInstanceCallback = (inView: boolean, options: ObserverInstanceCallbackOptions) => void;
...
class IOManager {
  ...
  handleChange = (entries: IntersectionObserverEntry[]) => {
    for (let index = 0; index < entries.length; index += 1) {
      ...
      if (instance) {
        instance.callback(isIntersecting, { expose: target.expose, moduleName: target.moduleName });
      }
    }
  };
