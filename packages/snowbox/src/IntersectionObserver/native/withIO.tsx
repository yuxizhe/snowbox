...
const withIO = (ScrollableComponent: typeof ScrollViewComponent): typeof IOScrollView => {
  class IOScrollableComponent extends PureComponent<IOScrollViewProps> {
    ...
    public getAllInViewTarget(isReset: boolean = false) {
      const result: any = [];
      this.manager.instanceMap.forEach((e) => {
        result.push(e.element);
        if (isReset) {
          e.element.expose.start = Date.now();
          e.element.expose.end = 0;
        }
      });
      return result;
    }

    public getAllInViewTargetBind = this.getAllInViewTarget.bind(this);
    ...
    public getScrollResponder = (): JSX.Element =>
      this.scroller.current?.getScrollResponder() as unknown as JSX.Element;

    public getScrollableNode = (): any => this.scroller.current?.getScrollableNode();

    public getInnerViewNode = (): any => this.scroller.current?.getInnerViewNode();
