 ...
 * @return {BaseMatrix} the resulted matrix.
 */
export function fromArray(array) {
  const m = new BaseMatrix();
  const a: number[] = Array.from(array);
 ...
 *
 * @param {BaseMatrix | DOMMatrix | CSSMatrix.JSONMatrix} m the source matrix to feed values from.
 * @return {BaseMatrix} the resulted matrix.
 */
function fromMatrix(m) {
  const keys = Object.keys(new BaseMatrix());
    ...
    .forEach((tf) => {
      ...
      if (prop === 'perspective' && x && [y, z].every((n) => n === undefined)) {
        ...
        if (['skewX', 'skewY'].includes(prop)) {
          ...
          const axeValues = [idx === 0 ? x : 0, idx === 1 ? x : 0, idx === 2 ? x : 0];
          // @ts-ignore unfortunately
          m = m[fn](...axeValues);
        }
      } else {
 ...
 *
 * @param {BaseMatrix} m1 the first matrix.
 * @param {BaseMatrix} m2 the second matrix.
 * @return {BaseMatrix} the resulted matrix.
 */
...
class BaseMatrix {
   ...
   *
   * @param {string | number[] | BaseMatrix | DOMMatrix} source
   * @return {BaseMatrix} the matrix instance
   */
   ...
   *
   * @param {BaseMatrix | DOMMatrix | CSSMatrix.JSONMatrix} m2 CSSMatrix
   * @return {BaseMatrix} The resulted matrix.
   */
  multiply(m2) {
